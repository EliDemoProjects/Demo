...
When using Developer Assist in **VS Code** with **GitHub Copilot**, there is a known issue that each time you call the Checkmarx MCP, the `mcp.json` file automatically opens in your workspace.
...
{% hint style="warning" %}
Once this workaround is implemented, the Checkmarx MCP will no longer start automatically each time you restart VS Code. Whenever you restart VS Code you will need to start the MCP manually, as follows:
